...
// La portada agrega 3 mundos independientes (Personal/Cerebro, Coaching,
// Personal/Finanzas) — un fallo leyendo uno no debe vaciar los otros dos,
// así que cada query se aísla en vez de un Promise.all que todo-o-nada.
...
export default async function Home() {
  ...
  const [tasks, reminders, clientes, transacciones] = await Promise.all([
    cargarOVacio(listTasks(), "tareas"),
    cargarOVacio(listReminders(), "recordatorios"),
    cargarOVacio(listClientes(), "clientes (coaching)"),
    cargarOVacio(listTransacciones(), "transacciones"),
  ]);
  ...
  return (
    <div className="px-10 py-10">
      ...
      <div className="mb-10">
        ...
        <HoyAccionesQueue clientes={clientes} />
      </div>
      ...
      <div className="mb-10 grid grid-cols-2 gap-8">
        <div>
          <div className="mb-3.5 flex items-center gap-2.5 text-[10px] tracking-[0.24em] text-gold-dim uppercase">
            Tareas de hoy
            <span className="rounded-full bg-[rgba(201,169,110,.12)] px-2 py-0.5 font-display text-[12px] text-gold-bright">{tareasHoy.length}</span>
            <span className="h-px flex-1 bg-line" />
        ...
        <div>
          <div className="mb-3.5 flex items-center gap-2.5 text-[10px] tracking-[0.24em] text-gold-dim uppercase">
            ...
            <span className="rounded-full bg-[rgba(201,169,110,.12)] px-2 py-0.5 font-display text-[12px] text-gold-bright">{recHoy.length}</span>
            <span className="h-px flex-1 bg-line" />
